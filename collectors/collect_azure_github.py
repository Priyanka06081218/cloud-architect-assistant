# collectors/collect_azure_github.py
#
# Collects README and Terraform files from high-quality Azure infrastructure repos.
...
import base64
import time
from config import GITHUB_TOKEN, RAW_AZURE_GITHUB
...
SEARCH_QUERIES = [
    "azure architecture terraform stars:>200",
    "azure aks production terraform stars:>100",
    "azure functions terraform stars:>100",
    "azure container apps terraform stars:>50",
    "azure well-architected terraform stars:>50",
    "azure cosmos-db terraform stars:>50",
    "azure service-bus terraform stars:>50",
    "azure kubernetes service production stars:>100",
]
...
def get_terraform_files(owner, repo):
    """Return up to 10 .tf files from the repo."""
...
def run():
    os.makedirs(RAW_AZURE_GITHUB, exist_ok=True)
    seen_repos = set()
    total      = 0
    ...
    for query in SEARCH_QUERIES:
        ...
        for repo in repos:
            ...
            out_path   = os.path.join(RAW_AZURE_GITHUB, f"{slug}.json")
    ...
    print(f"\nTotal Azure GitHub repos collected: {total}")
