# collectors/collect_gcp_stackoverflow.py
#
# Fetches GCP architecture questions + accepted answers from Stack Overflow.
# Same bulk-fetch pattern as collect_stackoverflow.py.
...
from config import STACKOVERFLOW_KEY, RAW_GCP_STACKOVERFLOW
...
def collect_for_tag(tag, pages=3):
    output_path = os.path.join(RAW_GCP_STACKOVERFLOW, f"{tag}.json")
...
def run():
    os.makedirs(RAW_GCP_STACKOVERFLOW, exist_ok=True)
    total = 0
    ...
    print(f"\nTotal GCP SO Q&A pairs collected: {total}")
